...
use dezmont765\yii2bundle\components\geometry\FileAlreadyExistsException;
use dezmont765\yii2bundle\components\geometry\FileNotFoundException;
use dezmont765\yii2bundle\components\geometry\FileNotSaveException;
use dezmont765\yii2bundle\components\geometry\GDNotInstalledException;
use dezmont765\yii2bundle\components\geometry\IllegalArgumentException;
use dezmont765\yii2bundle\components\geometry\InvalidFileException;
use dezmont765\yii2bundle\components\geometry\Point;
use dezmont765\yii2bundle\components\geometry\Rectangle;
use dezmont765\yii2bundle\components\geometry\Size;
use dezmont765\yii2bundle\components\geometry\UnsupportedFormatException;
...
class AcImage
{
    ...
    private static $correctPositions = [0, 1, 2, 3, 4];
    ...
    public function saveAsJPG($path) {
        if(!AcImageJPG::isSupport()) {
            throw new UnsupportedFormatException('JPG');
        }
    ...
    public function resize() // Size $s || $width, $height
    {
        ...
        if(count($args) == 2) // $width, $height
        {
            ...
            if(count($args) == 1 && $args[0] instanceof Size) {
                $size = $args[0];
    ...
    public function crop() // Rectangle $rect || $x, $y, $w, $h
    {
        ...
        if(count($a) == 4) {
            ...
            if(count($a) == 1 && $a[0] instanceof Rectangle) {
                $rect = $a[0];
            }
        }
    ...
    public function rotate($angle) {
        $this->setResource(imagerotate($this->resource, $angle, 0));
        $this->setSize(new Size($this->getHeight(),$this->getWidth()));
        return $this;
    }
}
